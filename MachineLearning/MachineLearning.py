#------------------------------------------------------------------------------------------------------------------------------------
#第 2 章: K近邻算法 http://ml.apachecn.org/mlia/knn/
收集数据：提供文本文件
...
def classify0(inX, dataSet, labels, k):
    ...
    sqDiffMat = diffMat ** 2
    # 将矩阵的每一行相加
    sqDistances = sqDiffMat.sum(axis=1) #1为行相加，0为列相加
    # 开方
...
def datingClassTest():
    ...
    # ax.scatter(datingDataMat[:, 1], datingDataMat[:, 2], 15.0 * array(datingLabels), 15.0 * array(datingLabels)) #第三个参数表示各点相对大小，值一样的点大小一样，第四个参数表示颜色，值一样的点颜色一样
    # plt.show()
    ...
    for i in range(numTestVecs):
        # 对数据测试
        classifierResult = classify0( [i], normMat[numTestVecs: m], datingLabels[numTestVecs: m], 5)
